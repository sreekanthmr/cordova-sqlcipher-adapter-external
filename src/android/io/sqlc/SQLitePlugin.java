...
public class SQLitePlugin extends CordovaPlugin {
     ...
     * @param dbName   The name of the database file
     */
    private SQLiteAndroidDatabase openDatabase(String dbname, String key, String dblocation, CallbackContext cbc, boolean old_impl, int dbid) throws Exception {
        try {
        ...
        private SQLiteAndroidDatabase openDatabase2(String dbname, String key, String dblocation, CallbackContext cbc, boolean old_impl) throws Exception {
        try {
            // ASSUMPTION: no db (connection/handle) is already stored in the map
            // [should be true according to the code in DBRunner.run()]
            File dbfile = getDatabaseFile(dbname, dblocation);
    ...
    private class DBRunner implements Runnable {
        ...
        public void run() {
            try {
                if (!oldImpl)
                    this.mydb = this.mydb1 = openDatabase(dbname, this.dbkey, dblocation, this.openCbc, this.oldImpl, this.dbid);
                else
                    this.mydb = openDatabase2(dbname, this.dbkey, dblocation, this.openCbc, this.oldImpl);
            } catch (Exception e) {
